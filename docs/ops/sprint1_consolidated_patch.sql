...
-- ------------------------------------------------------------------------------
-- 2. FLEXIBILIZAÇÃO DO SCHEMA (CHECK-IN RÁPIDO EM 30 SEGUNDOS)
-- ------------------------------------------------------------------------------
ALTER TABLE IF EXISTS clients ALTER COLUMN document DROP NOT NULL;
ALTER TABLE IF EXISTS clients ALTER COLUMN street DROP NOT NULL;
ALTER TABLE IF EXISTS clients ALTER COLUMN number DROP NOT NULL;
ALTER TABLE IF EXISTS clients ALTER COLUMN neighborhood DROP NOT NULL;
...
DO $$ BEGIN
    ALTER TABLE clients ADD COLUMN IF NOT EXISTS client_type client_type_enum DEFAULT 'B2C';
EXCEPTION WHEN OTHERS THEN null; END $$;

-- ------------------------------------------------------------------------------
-- 3. DROP COMPLETO DE TODAS AS ASSINATURAS ANTIGAS / SOBRECARREGADAS (PGRST203 FIX)
-- ------------------------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS idx_clients_whatsapp_clean ON clients(whatsapp);

-- ------------------------------------------------------------------------------
-- 5. RPC 1: RASTREAMENTO SEGURO POR TOKEN UUID (PORTAL DO CLIENTE - ZERO LEAK)
-- Retorna estritamente os dados do cliente, ocultando custos e margens de lucro.
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 6. RPC 2: RASTREAMENTO COM 2º FATOR (OS + WHATSAPP)
-- ------------------------------------------------------------------------------
...
BEGIN
    ...
    IF p_phone IS NOT NULL AND length(trim(p_phone)) > 0 AND trim(p_phone) != '0000' THEN
        ...
        IF v_token IS NULL THEN
            RETURN JSONB_BUILD_OBJECT('found', false, 'error', 'Dados divergentes. Confirme o número da OS e o WhatsApp cadastrado.');
        END IF;
    ELSE
        SELECT wo.public_tracking_token
        INTO v_token
...
-- ------------------------------------------------------------------------------
-- 7. RPC 3: CRIAÇÃO ATÔMICA DE OS (CHECK-IN 30S & NOVA OS WIZARD)
-- ------------------------------------------------------------------------------
...
BEGIN
    ...
    -- 1. Localiza ou cria o cliente no CRM Único
    ...
    -- 2. Gera sequencial de OS e token UUID único
...
-- ------------------------------------------------------------------------------
-- 8. RPC 4: ATUALIZAÇÃO DE ORÇAMENTO (WIZARD DE PRECIFICAÇÃO)
-- ------------------------------------------------------------------------------
...
BEGIN
    ...
    -- Mapeamento seguro de service type
...
-- ------------------------------------------------------------------------------
-- 9. RPC 5: AVANÇO DE STATUS DA OS (COCKPIT BANCADA COM TELEMETRIA QA)
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 10. RPC 6: APROVAÇÃO POR TOKEN (PORTAL DO CLIENTE)
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 11. RPC 7: CARREGAMENTO COMPLETO DO KANBAN (COCKPIT BANCADA)
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 12. RPC 8: MÉTRICAS 360° DO DASHBOARD
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 13. PERMISSÕES DE SCHEMA E SEQUÊNCIAS
-- ------------------------------------------------------------------------------
